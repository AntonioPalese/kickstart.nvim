...
-- NOTE: Le personalizzazioni rispetto a kickstart sono marcate con [PERSONALE],
--  così è facile ritrovarle quando si fa merge da upstream.
...
do
  ...
  vim.g.have_nerd_font = true -- [PERSONALE]
  ...
  vim.o.relativenumber = true -- [PERSONALE]
  ...
  -- [PERSONALE] Tab and indentation settings: 4 spaces
  --  NOTE: guess-indent (Section 4) may override these in files with a detectable indentation.
  vim.o.tabstop = 4 -- Number of spaces that a <Tab> counts for
  vim.o.shiftwidth = 4 -- Number of spaces for each indentation step
...
do
  ...
  -- [PERSONALE] Open a terminal split with the project's virtualenv activated
  local function open_term(split_cmd)
    vim.cmd(split_cmd .. ' | term')
    vim.fn.chansend(vim.b.terminal_job_id, 'conda deactivate;')
    vim.fn.chansend(vim.b.terminal_job_id, 'source .venv/bin/activate\n')
  end
  vim.keymap.set('n', '<leader>th', function() open_term 'split' end, { desc = '[T]erminal [H]orizontal' })
  vim.keymap.set('n', '<leader>tv', function() open_term 'vsplit' end, { desc = '[T]erminal [V]ertical' })
  ...
  -- [PERSONALE] kept enabled, as in the previous config
...
-- [PERSONALE] Find the project's Python interpreter: uv first, then $VIRTUAL_ENV, then system python3.
--  Used by pyright (Section 6) and nvim-dap-python (Section 11).
---@return string
local function find_python()
  local out = vim.fn.trim(vim.fn.system 'uv run which python')
...
do
  ...
  -- [PERSONALE] LazyGit inside Neovim (plenary is also installed by telescope)
  vim.pack.add { gh 'nvim-lua/plenary.nvim', gh 'kdheepak/lazygit.nvim' }
  ...
  require('which-key').setup {
    ...
    spec = {
      ...
      { '<leader>g', group = '[G]it' }, -- [PERSONALE]
      { '<leader>d', group = '[D]ebug' }, -- [PERSONALE]
  ...
  if vim.g.have_nerd_font then
    ...
    -- Used for backwards compatibility with plugins that require `nvim-web-devicons` (e.g. telescope.nvim, bufferline.nvim)
  ...
  -- [PERSONALE] Bufferline: buffers as tabs at the top of the screen
  --  NOTE: Placed after mini.icons, so the icons mock is already active.
  ...
  vim.keymap.set('n', '<leader>x', '<cmd>bdelete<CR>', { desc = 'Chiudi buffer' })
end
...
do
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      ...
      -- [PERSONALE] moved from <leader>th to <leader>ti, since <leader>th opens the terminal
  ...
  -- [PERSONALE] Treat .cu/.cuh files as CUDA, so clangd attaches to them
  vim.filetype.add {
    extension = {
      cu = 'cuda',
      cuh = 'cuda',
  ...
  local servers = {
    ...
    -- [PERSONALE] Python, using the project's interpreter (uv / virtualenv)
    pyright = {
  ...
  -- [PERSONALE] Mason has no clangd build for aarch64: use the system one (sudo apt install clangd)
  local ensure_installed = vim.tbl_filter(function(name) return name ~= 'clangd' end, vim.tbl_keys(servers or {}))
  vim.list_extend(ensure_installed, {
    -- You can add other tools here that you want Mason to install
    'tree-sitter-cli', -- [PERSONALE] needed by nvim-treesitter (main) to build parsers
    'ruff', -- [PERSONALE] Python formatter (used by conform)
  })
...
do
  ...
  require('conform').setup {
    ...
    -- [PERSONALE] Format on save everywhere except C/C++, which lack a well standardized coding style
    ...
    default_format_opts = {
      lsp_format = 'never', -- [PERSONALE] Only use the external formatters configured below
    },
    -- You can also specify external formatters in here.
    formatters_by_ft = {
      lua = { 'stylua' }, -- [PERSONALE]
      python = { 'ruff_organize_imports', 'ruff_fix', 'ruff_format' }, -- [PERSONALE]
      -- rust = { 'rustfmt' },
...
do
  ...
  local parsers = { 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'vim', 'vimdoc', 'python' } -- [PERSONALE] + python
...
do
  ...
  --
  require 'kickstart.plugins.debug' -- [PERSONALE] enabled (nvim-dap + nvim-dap-ui)
  require 'kickstart.plugins.indent_line' -- [PERSONALE] enabled
  -- require 'kickstart.plugins.lint' -- NOTE: nvim-lint is configured in Section 11 instead
  require 'kickstart.plugins.autopairs' -- [PERSONALE] enabled
  require 'kickstart.plugins.neo-tree' -- [PERSONALE] enabled
...
-- ============================================================
-- SECTION 11: [PERSONALE] PYTHON TOOLING
-- nvim-lint (mypy), nvim-dap-python
-- ============================================================
do
  -- [[ Linting ]]
  ...
  vim.api.nvim_create_autocmd({ 'BufEnter', 'BufWritePost', 'InsertLeave' }, {
    group = vim.api.nvim_create_augroup('personal-lint', { clear = true }),
    callback = function() lint.try_lint() end,
  })

  -- [[ Python debugging ]]
  --  NOTE: Must come after `kickstart.plugins.debug` (Section 10), which installs nvim-dap and nvim-dap-ui.
  vim.pack.add { gh 'mfussenegger/nvim-dap-python' }
  local dap = require 'dap'
  local dap_python = require 'dap-python'
  local python = find_python()

  -- Ignore .vscode/launch.json files, only use the configuration below
